...
export function SellPage() {
  ...
  const create = useMutation({ mutationFn: () => api.createListing({ title: form.title, description: form.description, priceMinor: Number(form.priceMinor), currency: form.currency, condition: form.condition, categoryId: form.categoryId, location: form.location, images: attachments.map((attachment) => ({ url: attachment.dataUrl, altText: attachment.name })) }), onSuccess: (result) => navigate(`/listings/${result.data.id}`), onError: (caught) => setError(caught instanceof Error ? caught.message : "Unable to create listing.") });
