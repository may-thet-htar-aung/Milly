...
export function RegisterPage() {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { await register(name, email, password); navigate("/"); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to create your account."); } finally { setBusy(false); } };
  return <AuthShell title="Make room for good finds" intro="Create an account to save listings and share what you no longer need."><form className="auth-form" onSubmit={submit}><label className="field-label">Your name<div className="input-icon"><UserRound size={17} /><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Your name" minLength={2} required /></div></label><label className="field-label">Email<div className="input-icon"><Mail size={17} /><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" required /></div></label><label className="field-label">Password<PasswordField value={password} onChange={setPassword} placeholder="At least 8 characters" /></label>{error && <div className="form-error">{error}</div>}<Button type="submit" size="lg" disabled={busy}>{busy ? "Creating account…" : "Create account"}</Button><p className="auth-switch">Already have an account? <Link to="/login">Sign in</Link></p></form></AuthShell>;
}
